...
export const enhance = (
  attr: number
): { check: number; delta: number; attr: number } => {
  const check = r(1, 100);
  let delta = 0;
  if (check > attr) {
    delta = r(1, 10);
...
export const computeDbBuild = ({
  str,
  siz,
}: Pick<Stats, "str" | "siz">): { db: string; build: number } | null => {
  const sum = str + siz;
...
export const computeHp = ({
  con,
  siz,
}: Pick<Characteristics, "con" | "siz">): number =>
