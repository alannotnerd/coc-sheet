...
class BackstoryForm extends React.Component<Props, State> {
  render() {
    const name = (name: keyof Backstory) => ({
      ...
      fullWidth: true,
      multiline: true,
      onEdited: (value: string) => this.props.change({ [name]: value }),
    });

    return (
      <Grid container spacing={16}>
        <Grid xs={12} md={8} item>
          <Input {...name("personalDescription")} />
        </Grid>
        <Grid xs={12} md={4} item>
          <Input {...name("ideologyOrBeliefs")} />
        </Grid>
        <Grid xs={12} md={4} item>
          <Input {...name("significantPeople")} />
        </Grid>
        <Grid xs={12} md={4} item>
          <Input {...name("meaningfulLocations")} />
        </Grid>
        <Grid xs={12} md={4} item>
          <Input {...name("treasuredPossessions")} />
        </Grid>
        <Grid xs={12} md={4} item>
          <Input {...name("traits")} />
        </Grid>
        <Grid xs={12} md={4} item>
          <Input {...name("injuriesAndScars")} />
        </Grid>
        <Grid xs={12} md={4} item>
          <Input {...name("phobiasAndManias")} />
        </Grid>
      </Grid>
    );
  }
...
const mapStateToProps = (state: Sheet): Pick<Props, "backstory"> => ({
