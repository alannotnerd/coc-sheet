...
class OccupationForm extends React.Component<Props, State> {
  ...
  handleSelect = (id: number) => {
    ...
    this.props.logger(
      infoRecord(`选择了 ${occupation.name} 为职业`, "SELECT_OCCUPATION")
    );
    this.handleClose();
  };
  ...
  render() {
    ...
    const occupationList = occupations.map((x, i) => (
      <ListItem key={i} onClick={() => this.handleSelect(i)} button>
        {x.name}
      </ListItem>
    ));
    const skillPoint = computeSkillPoint(
      this.props.attributes,
      occupation.skillPoint
    );
              ...
              <Grid container spacing={16} alignItems="center">
                ...
                <Grid item>
                  <IconButton onClick={this.handleOpen}>
                    <Icon fontSize="inherit">work</Icon>
                  </IconButton>
                </Grid>
              </Grid>
            </Grid>
...
const mapStateToProps = (
  state: Sheet
): Pick<Props, "occupation" | "attributes"> => ({
  occupation: state.occupation,
  attributes: state.stats,
});

const mapDispatchToProps = (
  dispatch: Dispatch
): Pick<Props, "change" | "logger"> => ({
  change: (x) => dispatch(editOccupation(x)),
